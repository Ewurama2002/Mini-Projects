...
const data = {
  'Year 1': {
    chapters: {
      " LISTENING AND SPEAKING ": ["Vowels", "Consonants"],
      " READING COMPREHENSION AND SUMMARY ": ["Reading skills", "Summary skills"],
    }
  },
  'Year 2': {
    chapters: {
  ...
  'Year 3': {
    chapters: {
      " LISTENING AND SPEAKING ": ["Speechwork", "Sentence Stress"],
      " READING COMPREHENSION AND SUMMARY ": ["Reading skills", "Summary skills"],
    }
  }
};
  ...
  const renderUnits = (units) => (
    units.map((unit, index) => (
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('unit1ex')} // Navigate to UnitPage
      >
        <Text>{unit}</Text>
      </TouchableOpacity>
    ))
